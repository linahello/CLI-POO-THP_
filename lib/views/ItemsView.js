...
class ItemsView {
  ...
  async show(item) {
    ...
    if (item.contact) {
      head.push("contact");
      colWidths.push(30);
      rows.push(chalk.blue(item.contact));
    }
  ...
  async itemForm({
    ...
    if (!this._controller.admin) {
      questions.push({
        name: "contact",
        type: "input",
        message: "Renseignez votre email@batard.com"
      })
    }
